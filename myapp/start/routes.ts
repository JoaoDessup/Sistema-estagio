...
Route.get(':id/vaga', 'VagasController.show')
  .middleware('auth:web')
  .where('id', /^[0-9]$/)
  .as('vagas.show')
...
Route.get('/perfil', 'EstudantesController.show').middleware('auth:web')
...
Route.get(':id/vaga/inscritos', 'VagasController.list')
  .middleware('auth:web')
  .where('id', /^[0-9]$/)
  .as('vagas.inscritos')

Route.post(':id/vaga/remove', 'VagasController.remove').as('vagas.remove').middleware('auth:web')
Route.post(':id/vaga/replace', 'VagasController.replace').as('vagas.replace').middleware('auth:web')
Route.get(':id/vaga/quiz', 'QuizzesController.show')
  .middleware('auth:web')
  .where('id', /^[0-9]$/)
  .as('quiz.show')
Route.post(':id/vaga/quiz', 'QuizzesController.store')
  .middleware('auth:web')
  .where('id', /^[0-9]$/)
  .as('quiz.store')
